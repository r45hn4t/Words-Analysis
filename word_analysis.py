...
for filen in os.listdir("./TD_cleaned"):
    with open(f"./TD_cleaned/{filen}",'r') as file:
        fresh_text = ""
        keep_word=['I','A',"\n",'a','of', 'to', 'in', 'it', 'is', 'be', 'as', 'at', 'so', 'we', 'he', 'by', 'or', 'on', 'do', 'if', 'me', 'my', 'up', 'an', 'go', 'no', 'us', 'am']
        for line in file.readlines():
            ...
            fresh_text+=line
        # print(fresh_text)
        spc=0;mpc=0;lpc=0;wc=0
        list_a=[];list_b1=[];list_b2=[];list_b3=[];list_b=[];wlc=[]
        for line in fresh_text.split("\n"):
            ...
            word_count = len(items)-(items.count("(.)")+items.count("(..)")+items.count("(...)")+items.count("[/]")+items.count("[//]"))
            if word_count<0:
                word_count = 0
        ...
        wc=sum(list_a);spc=sum(list_b1);mpc=sum(list_b2);lpc=sum(list_b3);tpc=sum(list_b)
    filename=filen[::-1][12:][::-1]
    TD_stat[f"{filename}"]=[["word_in_each_statment",wlc],['word_count',wc],["short_pause_count",spc],["medium_pause_count",mpc],["long_pause_count",lpc],["total_pause_count",tpc]]    

class DataAnalyser:
    def __init__(self,cleaned_file):
        self.cleaned_file = cleaned_file
    def numberofWords(self):
        filen = self.cleaned_file
        with open(f"./TD_cleaned/{filen}_cleaned.txt",'r') as file:
            fresh_text = ""
            keep_word=['I','A',"\n",'a','of', 'to', 'in', 'it', 'is', 'be', 'as', 'at', 'so', 'we', 'he', 'by', 'or', 'on', 'do', 'if', 'me', 'my', 'up', 'an', 'go', 'no', 'us', 'am']
            for line in file.readlines():
                line = line.replace(">","")
    ...
    def _str_(self):
        ...
        with open(f"./TD_cleaned/{filen}_cleaned.txt",'r') as file:
            ...
            wc=sum(list_a);spc=sum(list_b1);mpc=sum(list_b2);lpc=sum(list_b3);tpc=sum(list_b)
        tpc = spc+lpc+mpc
        return f"Total number of words is {wc} and total number of pauses is {tpc}"

# For testing the class,run the example code below:

da1 = DataAnalyser("TD3")
print(da1.numberofWords())
print(da1.numberofPauses())
print(da1._str_())
...
for key in TD_stat.keys():
    if key in ["TD1","TD5","TD9"]:
        ...
        axs[plot_count].set_title(f"Word in each statement in {key}.txt",fontsize=18)
